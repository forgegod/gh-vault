...
- `secrets check` verifies `GH_SECRET_*` and `GH_VAR_*` names in both `.env` and GitHub without modifying `.env`; local-secret-to-remote-variable mappings are successful migrations reported as `NAME -> GH_VAR_NAME`, while local-variable-to-remote-secret mappings are nonzero type drift reported as `GH_SECRET_NAME -> GH_VAR_NAME`. Remote-only secrets are reported as unset in `.env`. `variables import` reads repository variables with `gh variable list`, writes them as `GH_VAR_*` entries to `.env` or (when absent) `.env.example`, and replaces matching entries only with `--force`.
