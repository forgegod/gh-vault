...
- Profile references are honored by `secret sync` (resolves and pushes the stored token to GitHub Secrets), `secret export-act`, and `run-act` (writes the resolved token into the ephemeral `.secrets` file); `variable sync` ignores them because profile references are only valid for `secret` directives. `action_values` and `runtime_environment` both require a `VaultStore` to resolve profile references and raise `StoreError` naming the offending file:line when the referenced profile is not configured.
