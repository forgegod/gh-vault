...
def build_parser() -> argparse.ArgumentParser:
    ...
    secret_check = secrets.add_parser("check", help="verify declared Actions types on GitHub", description="Compare GH_SECRET_ and GH_VAR_ declarations with both GitHub Actions stores without changing .env."); secret_check.add_argument("--env-file", type=Path, default=Path(".env"), help="environment file path"); secret_check.add_argument("--repo", help="target repository; defaults to origin")
    variables = commands.add_parser("variables", help="manage repository variables", description="Import GitHub Actions Variables into the local GH_VAR_ convention.").add_subparsers(dest="variables_command", required=True)
    variable_import = variables.add_parser("import", help="import GitHub Variables into .env", description="Import repository Variables as GH_VAR_ entries without replacing local values unless forced."); variable_import.add_argument("--repo", help="source repository; defaults to origin"); variable_import.add_argument("--force", action="store_true", help="overwrite existing GH_VAR_ settings")
