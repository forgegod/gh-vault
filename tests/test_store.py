...
def test_migrate_legacy_copies_profiles_and_preserves_source(store: VaultStore) -> None:
    legacy_config = store.config_dir.parent / "github-token-safe" / "config.json"
        ...
        json.dumps(
            {
                "active": "release",
                "profiles": {
                    "read": {"scopes": ["contents:read"], "note": "read only"},
                    "release": {"scopes": ["contents:write"], "note": "release"},
                },
            }
        ),
        encoding="utf-8",
    ...
    secret_db = Path(os.environ["FAKE_SECRET_DB"])
    secret_db.write_text(
        json.dumps(
            {
                "github-token-safe/read": "legacy-read",
                "github-token-safe/release": "legacy-release",
            }
        ),
        encoding="utf-8",
    )
    ...
    assert store.active() == "release"
    assert store.get("read") == "legacy-read"
    assert store.get("release") == "legacy-release"
    assert json.loads(legacy_config.read_text(encoding="utf-8"))["active"] == "release"
    assert json.loads(secret_db.read_text(encoding="utf-8")) == {
        "github-token-safe/read": "legacy-read",
        "github-token-safe/release": "legacy-release",
        "gh-vault/read": "legacy-read",
        "gh-vault/release": "legacy-release",
    }
